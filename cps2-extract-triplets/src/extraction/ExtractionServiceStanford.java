...
public class ExtractionServiceStanford {


	
	public ExtractionServiceStanford() {
	
	}

	public RDFTriplet extractTriplet(Tree root) {
		/*
		...
		RDFTriplet triplet = new RDFTriplet(subjectTree.nodeString(), predicateTree.nodeString(), objectTree.nodeString());
	...
	private Tree findFirstNoun(Tree npSubtree) {
		...
		while(noun == null && currentChild < npSubtree.numChildren()) {
			...
			//String child = npSubtree.getChild(currentChild).value();
			
			
			if(npSubtree.getChild(currentChild).value().equalsIgnoreCase("NP"))
				return findFirstNoun(npSubtree.getChild(currentChild));
			else if(isNoun(npSubtree.getChild(currentChild).value()))
